...
.transform((dto, ctx): Publish => {
        ...
        if(!recipe_template_result.success) {
                ctx.addIssue({
                        code: "custom",
                        message: `Invalid recipe_template: ${z.prettifyError(recipe_template_result.error as z.ZodError)}`,
                        fatal: true,
                });
                throw z.NEVER;
        }
        ...
        if(!canvas_ids_result.success) {
                ctx.addIssue({
                        code: "custom",
                        message: `Invalid canvas_ids: ${z.prettifyError(canvas_ids_result.error as z.ZodError)}`,
                        fatal: true,
                });
                throw z.NEVER;
        }
        ...
        if(!recipe_details_result.success) {
                ctx.addIssue({
                        code: "custom",
                        message: `Invalid recipe_details: ${z.prettifyError(recipe_details_result.error as z.ZodError)}`,
                        fatal: true,
                });
                throw z.NEVER;
        }
        ...
        if(!task_publish_state_result.success) {
                ctx.addIssue({
                        code: "custom",
                        message: `Invalid task_publish_state: ${z.prettifyError(task_publish_state_result.error as z.ZodError)}`,
                        fatal: true,
                });
                throw z.NEVER;
        }
